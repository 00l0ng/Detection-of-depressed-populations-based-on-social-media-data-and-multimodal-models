...
import pandas as pd
import re
from transformers import BertTokenizer, BertModel  # 修改1：导入BERT
import os
from PIL import Image
import torchvision.transforms as transforms
from torch.utils.data import TensorDataset, DataLoader
from torchvision.models import vgg16  # 修改1：导入VGG16
...
def load_data(csv_file_path, image_folder, selected_users, label):
    ...
    # BERT相关设置
    tokenizer = BertTokenizer.from_pretrained("bert-base-chinese")  # 修改3：使用BERT的Tokenizer
    bert = BertModel.from_pretrained("bert-base-chinese")  # 修改4：使用BERT模型
    bert.eval()
    ...
    # 保留特征提取部分并添加全局平均池化
    vgg_features = nn.Sequential(*list(vgg.children())[:-2])  # 获取features部分
    vgg_avgpool = nn.AdaptiveAvgPool2d((1, 1))  # 添加全局平均池化
    vgg = nn.Sequential(vgg_features, vgg_avgpool)
    vgg.eval()
    ...
    def extract_image_features(image_path):
        ...
        return features.squeeze(0).squeeze(-1).squeeze(-1)  # 输出形状应为[512]
    ...
    for index, row in data.iterrows():
        ...
        if user_index in selected_users and user_index not in processed_users:
            ...
            with torch.no_grad():
                text_feature = bert(**encoded_input).last_hidden_state.mean(dim=1)  # 修改5：使用BERT提取文本特征
...
img_feature_dim = 512  # 修改5：VGG16输出维度改为512
